    ...
    def post(self, request):
        ...
        if request.query_params.get("student", None) is not None:
            ...
            if request.data.get("select", None) is not None:
                ...
                student.redeem = student.redeem + reward.coin
                student.save()
            return Response({"status": "success"}, status=HTTP_200_OK)
...
class CompleteView(APIView):
    def get(self, request):
        if request.query_params.get("goal", None) is not None:
            ...
            return Response(serializer.data, status=HTTP_200_OK)
        if request.query_params.get("student_id", None) is not None:
            goals = Goal.objects.filter(student=request.query_params.get("student")).filter(status="completed")
            serializers = GoalSerializer(goals, many=True)
            if serializers.is_valid():
                for serializer in serializers:
                    goal = Goal.objects.get(id=serializer.get("id"))
                    complete = goal.complete
                    complete_serializer = CompleteSearializer(complete)
                    serializer["coin"] = complete_serializer.get("coin")
                    serializer["reason"] = complete_serializer.get("explain")
                    serializer["date"] = complete_serializer.get("date")
                    user_serializer = UserSerializer(goal.reporter)
                    serializer["reporter"] = user_serializer.get("name")
                return Record(serializers, status=HTTP_200_OK)
